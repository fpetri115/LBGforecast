...
try:
    ...
    pass
from .modified_reference_spectra import vega, solar, sedpydir
...
class Filter(object):
    ...
    def obj_counts_hires(self, sourcewave, sourceflux, sourceflux_unc=0):
        ...
        def _fsps_tsum(yin, xin):
            grid_size = xin.shape[0]
            #  tsum = SUM( ABS((xin(2:nn)-xin(1:nn-1))) * &
            #  (yin(2:nn)+yin(1:nn-1))/2. )

            tsum = np.sum(abs(xin[1:grid_size]-xin[0:grid_size-1])*(yin[1:grid_size]+yin[0:grid_size-1])/2)
        ...
        #newtrans = newtrans/_fsps_tsum(newtrans/sourcewave, sourcewave)
        ...
        if True in (newtrans > 0.):
            ...
            counts = _fsps_tsum(sourcewave[ind] * newtrans[ind] *
                              sourceflux[..., ind],
                              sourcewave[ind])
